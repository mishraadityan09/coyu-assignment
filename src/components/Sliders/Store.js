...
const StoreSlide = () => {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto overflow-hidden my-8">
            <div className='my-2'>
                <div className='text-sm font-bold textblack my-2 uppercase'>Our Store</div>
                <div className='flex gap-2'>
                <Button label="Delhi" icon={active} active/>
                <Button label="Gurgaon" icon={inactive} />
                </div>
            </div>
            <div className="flex space-x-4 overflow-x-auto py-2  hide-scrollbar">
                {content.map((item, index) => (
                    <div
                        key={index}
                        className="flex-shrink-0 w-64 h-52 cursor-pointer transform transition duration-200 hover:scale-110"
